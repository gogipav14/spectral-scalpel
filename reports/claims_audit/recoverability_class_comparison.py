...
This combines:
- reports/claims_audit/precision_scaling_data.pkl       (hyperbolic, Maxwell)
- reports/claims_audit/precision_scaling_parabolic_truth_data.pkl  (parabolic, diffusion)
...
ROOT = "/home/gogip/github_repos/spectral-scalpel-private/reports/claims_audit"

# Load the previously-computed sweep results
with open(f"{ROOT}/precision_scaling_data.pkl", "rb") as f:
    hyp_data = pickle.load(f)            # [r64, r32] from verify_precision_scaling.py
with open(f"{ROOT}/precision_scaling_parabolic_truth_data.pkl", "rb") as f:
...
# CSV
out_csv = f"{ROOT}/recoverability_class_comparison.csv"
...
plt.tight_layout()
out_png = f"{ROOT}/recoverability_class_comparison.png"
